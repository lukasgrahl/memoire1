...
%	"Also, rather than imposing priors on the great ratios, C∗/Y ∗, I∗/K∗, K∗/Y ∗, and G∗/Y ∗, we fix the capital share, α, the depreciation rate, δ, and the share of government expenditure, g∗. This follows well established practices that pre-date Bayesian estimation of NKDSGE models." \cite{guerron-quintana_bayesian_2013}
	...
	\subsection{DSGE forecasts}
	While the Kalman filter for the posterior evaluation has been run on the entire data set starting in 1985 the following has been run on post 2010 data. This allows to assess the data's fit to recent events. 
	\subsubsection{RBC}
	
	The Kalman filter fed data on output and investment provided the best fit to data \ref{fig:rbc_kfil}. Looking at the filter output output and investment are closely traced and the real data lies within the 95\% confidence interval after 2010. This is also true for the recession in early 2020. This indicates a good fit of the model to data.
	
	Consumption, while not contained as an observable in the filter, also is relatively closely traced with similar accuracy of the confidence intervals. This points to even better model accuracy, as the dynamic system is able to replicate a non-included variable to decent extend. 
	
	The interest rate is less closely traced and predicted to be much more volatile by the Kalman filter than the case in reality. While this indicates that problems in the model specification it is important to note that the FED has pursued a notorious low interest rate policy for the sample duration. As such interest rate was not set by the dynamics contained in the model, and a good fit is unlikely.
	
	Labour is the only variable with bad fit. The 90\% confidence interval far-off reality and the filter even suggests a rise labour hours during the 2020 crisis. While in the case of Covid-19 one could argue that particular dynamics where at play \ref{fig:rbc_kfil_00} illustrates that even in earlier period recessionary unemployment is not well captured. The historic overview sheds a better light on the overall model fit. While post 2010 dynamics are well captured by the model, fit prior to this date is less accurate. Output and consumption data escapes the filter's 95\% confidence interval before and after the 2008 financial crisis.
	
	Forecasts
	
	The RBC forecasts seem reasonable in direction and as well as confidence. The 95\% confidence intervals naturally increase with distance to the point of prediction. However, especially at the one period ahead forecast they exihibt resonable bounds. Moreover, the point forecast with exeception of labour is closely tracing the real variable evolution with no systematic bias. Labour as explained in the above appears to incorrectly specified in the models functional form, yet not impeding the model in functioning. 
	...
	Naturally the BVAR does not provide confidence intervals like frequentist methods of forecasting. Instead model parameters are defined through a sampler, which in this case ran in four iterations. Sampling from these iterations that represent the posterior coefficient distribution allows to derive information about the range of possible outcomes. This range can then be interpreted as a confidence interval (ref). 
	A sample from the posterior draws is illustrated in blue \ref{fig:bvar}. The range of outcomes for output suggest bias in the forecast. They are note evenly distributed around the actual data series. Instead the range of outcomes underestimates output and also introduces additional volatility not matched by the actual observations. 
	This is also the case for all other variables, additional volatility or classicality is introduced systematically across all forecasts. The forecast for the real interest rate after over correcting even moves in the opposite direction. 
	...
	\begin{figure}
		\begin{center}
			\includegraphics[width=.5\textheight]{bvar_forecast.png}
