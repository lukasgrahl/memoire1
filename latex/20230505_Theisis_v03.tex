...
\begin{document}
	...
	The households are infinitely lived agents with homogeneous preferences, simultaneously solving a threefold optimisation problem. Given these properties the RBC introduces one representative agent solving the problem in place of a continuum of households.
	The agent is faced with an income from labour wage $W_t$, returns on last periods capital holdings $K_t{1-}$ and firm dividends $\Pi_t$. Income is divided between consumption, capital investment $I_t$ and leisure, while the leisure expenditure is implicitly made by choosing the number of labour hours $L_t$. The RBC assumes a Robinson-Crusoe economic setting in which households own firms and consequently receive dividends from profits. However, under perfect competition firm profits are zero and so is dividend income at equilibrium \cite{prescott_theory_1986}. The households utility is assumed to take the following form.
	\begin{equation}
