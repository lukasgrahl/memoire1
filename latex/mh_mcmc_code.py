...
param_posterio_list = {item: [mod.free_param_dict[item]] for item in /
                      model_params if item in prior_dist.keys()}
...
G_params = np.zeros(len(p)) + 1
G_shocks = np.zeros(len(s)) + 1
...
for i in range(0, n_runs):
    ...
    draw_dict = {
        ...
        'is_accepted': False,
        'parameters': {
            'prior': {item: None for item in model_params /
                      if item in prior_dist.keys()},
            'prior_pdf_p': {item: None for item in model_params /
                            if item in prior_dist.keys()},
            'posterior': {item: None for item in model_params /
                          if item in prior_dist.keys()}
        },
        'shocks': {
            'prior': {item: None for item in shock_names},
            'prior_pdf_s': {item: None for item in shock_names /
                            if item in prior_dist.keys()},
    ...
    prior = np.array(list(old_posterior_p.values()) + multivariate_normal(
        list([0] * len(old_posterior_p)), scaling_factor * G_params).rvs())
    
    # prior for shocks
    shocks = np.array(list(old_posterior_s.values()) + multivariate_normal(
        list([0] * len(old_posterior_s)), scaling_factor * G_shocks).rvs())
    
    # put priors into dictionary for further process
    prior, shocks = dict(zip(old_posterior_p.keys(), prior)),\
    ...
    H, Z, T, R, QN, zs = set_up_kalman_filter(R=R, T=T,
                                              observed_data=train[observed_vars].values,
